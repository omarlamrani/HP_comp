...
{
    int i, j;
    int jstart, jend, jrange;

    /* Calculate the range of rows to be computed by this process */
    jrange = jmax / nprocs;
    jstart = nprocs * jrange + 1;
    jend = (proc + 1) * jrange;
    if (proc == nprocs - 1) {
        /* Last process computes remaining rows */
        jend = jmax;
        jrange = jend - jstart + 1;
    }

    /* Compute RHS for each process's part of the grid */
    for (i = 1; i <= imax; i++) {
    ...
    /* Combine the RHS from all processes */
    if (nprocs > 1) {
        for (i = 1; i <= imax; i++) {
            MPI_Allgather(MPI_IN_PLACE, 0, MPI_FLOAT, &rhs[i][1], jrange,
    ...
    char **flag, int imax, int jmax, float del_t, float delx, float dely)
{
    ...
    // Determine the subdomain assigned to this process
    int subdomain_size = imax / nprocs;
    int remainder = imax % nprocs;
    if (proc < remainder) {
        istart = proc * (subdomain_size + 1) + 1;
        iend = istart + subdomain_size;
    } else {
        istart = proc * subdomain_size + remainder + 1;
        iend = istart + subdomain_size - 1;
    }

    // Exchange ghost cells with neighboring processes
    if (proc > 0) {
        MPI_Send(&u[istart][0], jmax+2, MPI_FLOAT, proc-1, 0, MPI_COMM_WORLD);
        MPI_Recv(&u[istart-1][0], jmax+2, MPI_FLOAT, proc-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    if (proc < nprocs-1) {
        MPI_Send(&u[iend][0], jmax+2, MPI_FLOAT, proc+1, 0, MPI_COMM_WORLD);
        MPI_Recv(&u[iend+1][0], jmax+2, MPI_FLOAT, proc+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }

    // Compute velocities for the subdomain assigned to this process
    ...
    // Exchange velocities at the boundaries with neighboring processes
    if (proc > 0) {
        MPI_Send(&u[istart][1], jmax, MPI_FLOAT, proc-1, 0, MPI_COMM_WORLD);
        MPI_Recv(&u[istart-1][1], jmax, MPI_FLOAT, proc-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    if (proc < nprocs-1) {
        MPI_Send(&u[iend][1], jmax, MPI_FLOAT, proc+1, 0, MPI_COMM_WORLD);
        MPI_Recv(&u[iend+1][1], jmax, MPI_FLOAT, proc+1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
}
