...
void Diffusion::init()
{
    ...
    // double pre_forloop_run = omp_get_wtime();
    if(!subregion.empty()) {
        #pragma omp parallel for collapse(2) schedule(guided)//private(u0)
        // int num_threads = omp_get_num_threads();
        // std::cout << "number_threads Diffuse_init " << num_threads << std::endl;
        ...
        // int num_threads = omp_get_num_threads();
        // std::cout << "number_threads Diffuse_init " << num_threads << std::endl;
        // outer loop executed in parallel by multiple threads 
        //improve the performance for large values of x_max and y_max.
        for (int j = 0; j < y_max+2; j++) {
            for (int i = 0; i < x_max+2; i++) {
                u0[i+j*nx] = 0.0;
            }
    ...
    // double post_forloop_run = omp_get_wtime();

    // printf("init_diffusion took %g seconds\n", post_forloop_run - pre_forloop_run);
