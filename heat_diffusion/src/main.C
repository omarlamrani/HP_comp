...
{
    // checks argc > 2, make sure dqn is tested with a file not empty
    if (argc != 2) {
    ...
    double pre_driver_run = omp_get_wtime();

   // #pragma omp parallel
    //{
    driver.run();
    //}
    double post_driver_run = omp_get_wtime();

    printf("driver.run() took %g seconds\n", post_driver_run - pre_driver_run);
    return 0;
}
